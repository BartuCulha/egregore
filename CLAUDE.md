...
- **Worktree mode** (`claude --worktree`): Each session gets an isolated worktree under `.claude/worktrees/`. The greeting shows `· worktree` in the branch status. Branch operations use `git branch -m` (rename) instead of `git checkout -b` (create). This prevents concurrent sessions from interfering with each other's branches.
