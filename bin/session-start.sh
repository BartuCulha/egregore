...
if [ "$IN_WORKTREE" != "true" ]; then
  ...
  if [ -n "$UPSTREAM_NEW" ]; then
    ...
    # Check for uncommitted local changes OR committed local changes ahead of upstream
    LOCAL_FRAMEWORK_DIRTY=$(git diff -- bin/ .claude/commands/ CLAUDE.md skills/ 2>/dev/null || true)
    LOCAL_FRAMEWORK_AHEAD=$(git log upstream/main..HEAD --oneline -- bin/ .claude/commands/ CLAUDE.md skills/ 2>/dev/null || true)
    if [ -n "$LOCAL_FRAMEWORK_DIRTY" ] || [ -n "$LOCAL_FRAMEWORK_AHEAD" ]; then
      echo "  ⟳ Local framework changes detected — skipping auto-update"
...
if [ "$IN_WORKTREE" != "true" ]; then
  ...
  # v3: add --worktree flag for session isolation (version-gated on Claude Code >= 2.1.50)
  if [ "$ALIAS_VERSION" -lt 3 ] 2>/dev/null; then
    ...
    if [ "$(printf '%s\n' "$CLAUDE_MIN" "$CLAUDE_VER" | sort -V | head -1)" = "$CLAUDE_MIN" ]; then
      for profile in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"; do
        if [ -f "$profile" ] && grep -q "$SCRIPT_DIR" "$profile" 2>/dev/null; then
          sed -i.bak 's|&& claude "start"|\&\& claude --worktree -- "start"|g; s|&& claude -w "start"|\&\& claude --worktree -- "start"|g' "$profile" 2>/dev/null && rm -f "${profile}.bak" || true
        fi
      done
      if [ -f "$STATE_FILE" ] && jq . "$STATE_FILE" >/dev/null 2>&1; then
